    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MusicItemViewHolder) {
            ...
            if (Objects.equals(list.get(position).getGender(), "male")) {
                ((MusicItemViewHolder) holder).icon.setImageResource(R.drawable.ic_round_person_24);
            } else {
                ((MusicItemViewHolder) holder).icon.setImageResource(R.drawable.ic_baseline_person_2_24);
            }
        }
    }
